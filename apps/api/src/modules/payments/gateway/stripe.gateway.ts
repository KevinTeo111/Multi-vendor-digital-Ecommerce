...
export class StripePaymentGateway implements PaymentGateway {
  ...
  async syncPlan(plan: PlanLike): Promise<{ gatewayPlanId: string }> {
    if (plan.gatewayPlanId) {
      try {
        ...
        const sameInterval =
          existing.recurring?.interval === (plan.interval === 'YEAR' ? 'year' : 'month');
        ...
        this.logger.warn(
          `Stripe price ${plan.gatewayPlanId} not reusable: ${(err as Error).message}`,
        );
      }
    }
  ...
  async parseWebhook(rawBody: Buffer, headers: WebhookHeaders): Promise<NormalizedWebhookEvent> {
    if (!this.webhookSecret)
      throw new WebhookRejectedError('STRIPE_WEBHOOK_SECRET is not configured');
  ...
  private async normalize(event: Stripe.Event): Promise<NormalizedWebhookEvent> {
    ...
    switch (event.type) {
      case 'checkout.session.completed': {
        ...
        if (session.mode === 'subscription') {
          ...
          return {
            ...
            eventId,
            gatewaySubscriptionId: session.id,
            newGatewaySubscriptionId: subId,
            ...period,
          };
        }
      ...
      case 'invoice.payment_failed': {
        ...
        return subId
          ? { kind: 'subscription.past_due', eventId, gatewaySubscriptionId: subId }
          : { kind: 'ignored', eventId, type: event.type };
      }
  ...
  private paid(eventId: string, session: Stripe.Checkout.Session): NormalizedWebhookEvent {
    ...
    const method = (session as unknown as { payment_method_types?: string[] })
  ...
  private async subscriptionPeriod(
    ...
    try {
      ...
      this.logger.warn(
        `Could not read subscription period for ${subscriptionId}: ${(err as Error).message}`,
      );
      return {};
    }
  }
