...
export function ProductCard({ product }: { product: ProductCardData }) {
  ...
  const badge =
    product.salesCount >= 10
      ...
      : product.publishedAt &&
          Date.now() - new Date(product.publishedAt).getTime() < NEW_WINDOW_DAYS * 86_400_000
  ...
  return (
    <article className="group relative flex flex-col overflow-hidden rounded-2xl border border-slate-200/80 bg-white shadow-card transition hover:-translate-y-0.5 hover:shadow-lg hover:shadow-brand-500/10">
      <Link
        href={href}
        className="relative block aspect-[4/3] w-full overflow-hidden bg-navy-900"
      ...
      >
        {product.thumbnailUrl ? (
          ...
          <div className="bg-hero flex h-full flex-col items-center justify-center gap-1 text-slate-400">
            <span className="text-brand-gradient-light text-lg font-bold">
        ...
        {badge && (
          <span
            className={`absolute left-3 top-3 rounded-full px-2.5 py-1 text-[11px] font-bold ${badge.className}`}
          >
            {badge.label}
          </span>
        )}
      ...
      <div className="flex flex-1 flex-col gap-1 p-4">
        <Link
          href={href}
          className="line-clamp-2 text-sm font-semibold text-navy-900 hover:text-brand-600"
        >
          {product.title}
        </Link>
        ...
        <div className="mt-auto flex items-center justify-between pt-3">
          ...
          <Link
            href={href}
            className="arrow-nudge flex h-9 w-9 items-center justify-center rounded-full border border-brand-500 text-brand-600 transition group-hover:bg-brand-500 group-hover:text-white"
            aria-label={product.title}
          >
