...
export function LanguageToggle({ className = '', dark = false }: { className?: string; dark?: boolean }) {
  ...
  return (
    <div className={`inline-flex items-center rounded-full p-0.5 text-xs font-bold ${dark ? 'bg-white/10' : 'bg-slate-100'} ${className}`} role="group" aria-label={t('nav.language')}>
