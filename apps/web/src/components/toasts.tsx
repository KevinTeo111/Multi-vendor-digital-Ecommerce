...
export function ToastProvider({ children }: { children: ReactNode }) {
  ...
  return (
    <ToastContext.Provider value={value}>
      {children}
      <div
        className="pointer-events-none fixed bottom-4 right-4 z-[60] flex w-[min(92vw,380px)] flex-col gap-2"
        aria-live="polite"
      >
        {toasts.map((t) => (
          <div
          ...
          >
            <span
              className={`mt-0.5 flex h-6 w-6 shrink-0 items-center justify-center rounded-full ${t.tone === 'error' ? 'bg-rose-100 text-rose-600' : t.tone === 'success' ? 'bg-emerald-100 text-emerald-600' : 'bg-brand-50 text-brand-600'}`}
            >
