      ...
      <section className="bg-hero text-white">
        <div className="mx-auto grid max-w-7xl gap-10 px-4 py-14 sm:px-6 lg:grid-cols-[1.1fr_1fr] lg:py-24">
          <div>
            ...
            <form
              action="/products"
              className="mt-8 flex max-w-xl items-center rounded-full bg-white p-1.5 shadow-glow"
            >
              <input
                name="search"
              ...
              <button
                type="submit"
                className="bg-brand-gradient flex h-10 w-10 shrink-0 items-center justify-center rounded-full text-white"
                aria-label={t('common.search')}
              >
                <SearchIcon size={18} />
              </button>
            ...
            <div className="relative grid grid-cols-2 gap-4 p-4">
              {floating.map((p, i) => {
                ...
                return (
                  ...
                  >
                    <span
                      className={`flex h-11 w-11 shrink-0 items-center justify-center rounded-xl ${FLOAT_COLORS[i % FLOAT_COLORS.length]}`}
                    >
                      <Icon size={20} />
                    </span>
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6">
        ...
        <section className="py-6">
          ...
          {featured && featured.total > 8 && (
            <div className="mt-8 text-center">
              <Link
                href="/products"
                className="bg-brand-gradient group inline-flex items-center gap-2 rounded-full px-6 py-3 text-sm font-semibold text-white shadow-glow"
              >
                {t('home.viewAllProducts', { count: featured.total })}{' '}
          ...
          <section className="py-10">
            <div className="mb-4 flex flex-wrap items-end justify-between gap-4">
              ...
              <Link
                ...
                className="bg-brand-gradient group inline-flex items-center gap-2 rounded-full px-5 py-2.5 text-sm font-semibold text-white shadow-glow"
              >
            ...
            <div className="border-t border-slate-200">
              {latest.items.map((p) => (
                <Link
                  key={p.id}
                  href={`/products/${p.slug}`}
                  className="group flex items-center gap-4 border-b border-slate-200 py-4 transition hover:bg-brand-50/40 sm:gap-6"
                >
                  <span className="w-24 shrink-0 text-xs text-slate-500 sm:w-28 sm:text-sm">
                    {formatDate(p.publishedAt, false, locale)}
                  </span>
                  <span className="hidden w-32 shrink-0 text-xs font-semibold text-brand-600 sm:block">
                    {p.category.name}
                  </span>
        ...
        <section className="py-8">
          <div className="grid gap-8 rounded-3xl border border-brand-100 bg-gradient-to-br from-brand-50 via-white to-cyan-400/10 p-8 lg:grid-cols-[1.2fr_1fr] lg:p-12">
            <div>
              ...
              <Link
                href="/products"
                className="bg-brand-gradient group mt-6 inline-flex items-center gap-2 rounded-full px-6 py-3 text-sm font-semibold text-white shadow-glow"
              >
        ...
        <section className="pb-6 pt-2">
          <div className="flex flex-col gap-6 rounded-3xl bg-navy-900 px-8 py-8 text-white lg:flex-row lg:items-center lg:px-10">
            ...
            <Link
              href="/plans"
              className="bg-brand-gradient group inline-flex items-center gap-2 self-start rounded-full px-6 py-3 text-sm font-semibold shadow-glow lg:self-center"
            >
...
function Trust({ icon, title, text }: { icon: React.ReactNode; title: string; text: string }) {
  return (
    <li className="flex items-center gap-3">
      <span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl border border-white/15 bg-white/10 text-cyan-400">
        {icon}
      </span>
      <span>
    ...
    <Link
      ...
      className={`flex min-w-[120px] flex-col items-center gap-2 rounded-2xl border px-4 py-4 text-center text-xs font-medium transition ${
        active
          ...
          : 'border-slate-200 bg-white text-slate-700 shadow-sm hover:border-brand-500 hover:text-brand-600'
      }`}
    >
      <span
        className={`flex h-10 w-10 items-center justify-center rounded-xl ${active ? 'bg-white text-brand-600' : 'bg-slate-50 text-slate-700'}`}
      >
        {icon}
      </span>
      {label}
