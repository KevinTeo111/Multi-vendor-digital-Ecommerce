    ...
    <>
      <section className="bg-hero text-white">
        <div className="mx-auto flex max-w-7xl flex-col gap-6 px-4 py-12 sm:flex-row sm:items-center sm:px-6">
          <span className="bg-brand-gradient flex h-20 w-20 shrink-0 items-center justify-center rounded-3xl text-3xl font-black shadow-glow">
            {vendor.storeName[0]}
          </span>
          <div className="min-w-0 flex-1">
            ...
            {vendor.description && (
              <p className="mt-2 max-w-2xl text-sm text-slate-300">{vendor.description}</p>
            )}
          </div>
          <dl className="flex gap-8 text-center">
            ...
            <Metric
              value={formatDate(vendor.createdAt, false, locale)}
              label={t('store.memberSince')}
              small
            />
          </dl>
        </div>
      ...
      <PageContainer>
        <h2 className="mb-5 text-2xl font-extrabold tracking-tight text-navy-900">
          {t('store.products')}
        </h2>
