...
export default async function StorePage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const [vendor, products] = await Promise.all([serverApi<VendorPublic>(`/vendors/${slug}`), serverApi<Paginated<ProductCard>>('/products', { vendor: slug, pageSize: 48 })]);
  if (!vendor) notFound();
        ...
        <div className="mx-auto flex max-w-7xl flex-col gap-6 px-4 py-12 sm:flex-row sm:items-center sm:px-6">
          ...
          <div className="min-w-0 flex-1">
            <h1 className="flex items-center gap-2 text-3xl font-extrabold tracking-tight">
              ...
              <BadgeCheckIcon size={22} className="shrink-0 text-cyan-400" />
            </h1>
          ...
          <dl className="flex gap-8 text-center">
            <Metric value={vendor.productCount} label={t('store.products')} />
            <Metric value={totalSales} label={t('store.sales')} />
            <Metric value={formatDate(vendor.createdAt, false, locale)} label={t('store.memberSince')} small />
          </dl>
        </div>
      </section>

      <PageContainer>
        <h2 className="mb-5 text-2xl font-extrabold tracking-tight text-navy-900">{t('store.products')}</h2>
        {products && products.items.length > 0 ? (
          <ProductGrid products={products.items} />
        ) : (
          <div className="rounded-2xl border border-dashed border-slate-300 bg-white p-14 text-center text-sm text-slate-500">{t('store.noProducts')}</div>
        )}
      </PageContainer>
    </>
  );
