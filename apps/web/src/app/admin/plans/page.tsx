...
export default function AdminPlansPage() {
  ...
  const save = async (e: FormEvent) => {
    ...
    const ok = await action.run(() =>
      editing.id ? api(`/admin/plans/${editing.id}`, { method: 'PATCH', body }) : api('/admin/plans', { method: 'POST', body }),
    );
    if (ok !== undefined) {
      setEditing(null);
      list.reload();
  ...
  const deactivate = async (p: Plan) => {
    if (!confirm(`Deactivate ${p.name}? Existing subscribers keep it; new vendors cannot choose it.`)) return;
  ...
  return (
    <div>
      <PageHeader title="Plans" description="Commission, listing limits and weekly withdrawal allowance per plan." actions={<Button onClick={() => setEditing({ id: null, form: empty })}>New plan</Button>} />
      ...
      <Card>
        <Table headers={['Plan', 'Price', 'Commission', 'Max products', 'Withdrawals/wk', 'Subscribers', 'Status', '']}>
          {list.data.map((p) => (
            <tr key={p.id}>
              <Td>
              ...
              <Td>
                {p.priceCents === 0 ? 'Free' : formatMoney(p.priceCents, p.currency)} / {p.interval.toLowerCase()}
              </Td>
              ...
              <Td>
                <Badge status={p.isActive ? 'ACTIVE' : 'CANCELED'}>{p.isActive ? 'Active' : 'Inactive'}</Badge>
              </Td>
              <Td>
      ...
      <Modal open={editing !== null} title={editing?.id ? 'Edit plan' : 'New plan'} onClose={() => setEditing(null)}>
        {editing && (
          <form onSubmit={save} className="space-y-3">
            ...
            <label className="flex items-center gap-2 text-sm">
              <input type="checkbox" checked={editing.form.isActive} onChange={set('isActive')} /> Available for new subscriptions
            </label>
