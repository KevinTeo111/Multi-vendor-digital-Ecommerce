...
export default function AdminWithdrawalsPage() {
  ...
  const approve = async (w: Withdrawal) => {
    ...
    if (!confirm(manual ? t('admin.approveManualConfirm', vars) : t('admin.approveGatewayConfirm', vars))) return;
  ...
  const markPaid = async () => {
    if (!paying) return;
    const ok = await action.run(() => api(`/admin/finance/withdrawals/${paying.id}/mark-paid`, { method: 'POST', body: { reference: reference || undefined, notes: notes || undefined } }));
    if (ok !== undefined) {
      setPaying(null);
      setReference('');
      setNotes('');
  ...
  return (
    <div>
      <PageHeader title={t('admin.withdrawalsTitle')} description={manual ? t('admin.manualDescription') : t('admin.gatewayDescription')} />
      {action.error && (
        <div className="mb-4">
      ...
      >
        {list.loading && !list.data ? (
          ...
          <>
            <Table headers={[t('vendor.requested'), t('admin.vendor'), t('admin.payoutTo'), t('common.amount'), t('common.status'), t('common.actions')]}>
              {list.data.items.map((w) => (
                <tr key={w.id}>
                    ...
                    {w.vendor?.payoutDetails ? (
                      w.vendor.payoutDetails.type === 'PIX' ? (
                        <>
                          ...
                          <div>
                            {w.vendor.payoutDetails.holderName} · {w.vendor.payoutDetails.holderDocument}
                          </div>
                        </>
                      ) : (
                        <>
                          <div className="font-medium text-slate-700">
                            {w.vendor.payoutDetails.bankCode} · Ag {w.vendor.payoutDetails.branch} · Cc {w.vendor.payoutDetails.accountNumber}
                          </div>
                          <div>
                            {w.vendor.payoutDetails.holderName} · {w.vendor.payoutDetails.holderDocument}
                          </div>
                  ...
                  <Td>
                    ...
                    {w.paidAt && <div className="mt-1 text-xs text-slate-500">{t('admin.paidOn', { date: formatDate(w.paidAt) })}</div>}
                  </Td>
                  <Td>
      ...
      <Modal open={paying !== null} title={t('admin.markPaidTitle')} onClose={() => setPaying(null)}>
        {paying && (
          <div className="space-y-3">
            <p className="text-sm text-slate-600">{t('admin.markPaidText', { amount: formatMoney(paying.amountCents), vendor: paying.vendor?.storeName ?? '' })}</p>
      ...
      <Modal open={rejecting !== null} title={t('admin.rejectWithdrawal')} onClose={() => setRejecting(null)}>
        <p className="mb-2 text-sm text-slate-500">{t('admin.rejectWithdrawalText')}</p>
        <Textarea value={reason} onChange={(e) => setReason(e.target.value)} rows={3} placeholder={t('admin.reasonPlaceholder')} />
