...
function SubscriptionView() {
  ...
  const subscribe = async (planId: string) => {
    const res = await action.run(() =>
      api<{ checkoutUrl: string | null }>('/vendor/subscription', {
        method: 'POST',
        body: { planId },
      }),
    );
  ...
  return (
    <div className="space-y-6">
      ...
      {returned === 'success' && !current && (
        <Alert tone="info">{t('vendor.checkoutProcessing')}</Alert>
      )}
      {returned === 'canceled' && !current && (
        <Alert tone="warning">{t('vendor.checkoutCanceled')}</Alert>
      )}
        ...
        >
          <div className="flex flex-wrap items-center gap-3 text-sm">
            ...
            <span className="text-slate-500">
              {current.status === 'CANCELED' ? t('vendor.accessUntil') : t('vendor.renews')}{' '}
        ...
        {plans.data.map((plan) => {
          ...
          return (
            <Card key={plan.id} className={highlight === plan.slug ? 'ring-2 ring-brand-500' : ''}>
              ...
              >
                {isCurrent
                  ? t('vendor.currentPlanButton')
                  : current
                    ? t('vendor.switchPlan')
                    : t('vendor.choosePlanButton')}
              </Button>
              {plan.priceCents > 0 && (
                <p className="mt-2 text-center text-xs text-slate-500">
                  {t('vendor.securePaymentPage')}
                </p>
              )}
            </Card>
          );
