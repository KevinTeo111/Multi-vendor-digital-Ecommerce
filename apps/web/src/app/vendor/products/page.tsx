...
export default function VendorProductsPage() {
  ...
  useRealtimeEvent('product.status', (p) => {
    list.setData((prev) =>
      prev ? { ...prev, items: prev.items.map((item) => (item.id === p.productId ? { ...item, status: p.status as VendorProduct['status'], rejectionReason: (p.reason as string | null) ?? null } : item)) } : prev,
    );
  });

  const act = async (id: string, verb: 'submit' | 'unpublish' | 'delete') => {
    if (verb === 'delete' && !confirm(t('vendor.deleteConfirm'))) return;
    const ok = await action.run(() => (verb === 'delete' ? api(`/vendor/products/${id}`, { method: 'DELETE' }) : api(`/vendor/products/${id}/${verb}`, { method: 'POST' })));
    if (ok !== undefined) list.reload();
  };

  return (
    <div>
      <PageHeader title={t('vendor.productsTitle')} actions={<LinkButton href="/vendor/products/new" arrow>{t('vendor.newProductButton')}</LinkButton>} />
      {action.error && (
        <div className="mb-4">
      ...
      >
        {list.loading && !list.data ? (
          ...
          <EmptyState title={t('vendor.noProducts')} description={t('vendor.noProductsText')} action={<LinkButton href="/vendor/products/new" arrow>{t('vendor.newProductButton')}</LinkButton>} />
        ) : (
          <>
            <Table headers={[t('vendor.product'), t('vendor.price'), t('common.status'), t('store.sales'), t('vendor.updated'), t('common.actions')]}>
              {list.data.items.map((p) => (
                <tr key={p.id}>
                  <Td>
                    <Link href={`/vendor/products/${p.id}`} className="font-medium text-navy-900 hover:text-brand-600">
                      {p.title}
                    </Link>
                    <div className="text-xs text-slate-500">
                      {p.category.name} · {t('vendor.filesCount', { count: p.fileCount ?? 0 })}
                    </div>
                    {p.status === 'REJECTED' && p.rejectionReason && <div className="mt-1 text-xs text-rose-600">{t('vendor.rejectedReason', { reason: p.rejectionReason })}</div>}
                  </Td>
