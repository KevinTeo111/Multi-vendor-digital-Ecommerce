...
export default function NewProductPage() {
  ...
  const create = async (values: ProductFormValues) => {
    const product = await action.run(() =>
      api<VendorProduct>('/vendor/products', { method: 'POST', body: values }),
    );
  ...
  return (
    <div>
      <PageHeader
        title={t('vendor.newProductTitle')}
        description={t('vendor.newProductDescription')}
      />
      <Card>
        <ProductForm
          ...
          busy={action.busy}
          error={action.error}
          submitLabel={t('vendor.createDraft')}
        />
      </Card>
    </div>
  );
