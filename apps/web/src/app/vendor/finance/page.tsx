...
export default function VendorFinancePage() {
  ...
  const withdrawals = useFetch<Paginated<Withdrawal>>('/vendor/finance/withdrawals', {
    page: wPage,
    pageSize: 10,
  });
  const [lPage, setLPage] = useState(1);
  const ledger = useFetch<Paginated<LedgerEntry>>('/vendor/finance/ledger', {
    page: lPage,
    pageSize: 15,
  });
  ...
  const request = async () => {
    ...
    const ok = await action.run(() =>
      api('/vendor/finance/withdrawals', {
        method: 'POST',
        body: cents ? { amountCents: cents } : {},
      }),
    );
    if (ok !== undefined) {
      setAmount('');
    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        <Stat
          ...
          value={formatMoney(balance.data.availableCents)}
          hint={t('vendor.minWithdrawal', { amount: formatMoney(e.minWithdrawalCents) })}
        />
        <Stat
          label={t('vendor.pendingBalance')}
          value={formatMoney(balance.data.pendingCents)}
          hint={t('vendor.clearsAutomatically')}
        />
      </div>
      ...
      <Card title={t('vendor.requestWithdrawal')}>
        ...
        {e.ok ? (
          <div className="flex flex-col gap-3 sm:flex-row sm:items-end">
            <Field label={t('vendor.amountOptional')}>
              <Input
                type="number"
                min={e.minWithdrawalCents / 100}
                step="0.01"
                value={amount}
                onChange={(ev) => setAmount(ev.target.value)}
                placeholder={(e.availableCents / 100).toFixed(2)}
              />
            </Field>
        ...
        <p className="mt-3 text-xs text-slate-500">
          {t('vendor.weeklyUsed', { used: e.requestsInLastWeek, limit: e.withdrawalsPerWeek })}
        </p>
      </Card>
