    ...
    <>
      <section className="bg-hero text-white">
        <div className="mx-auto max-w-3xl px-4 py-14 text-center sm:px-6">
          <span className="inline-flex rounded-full border border-white/15 bg-white/10 px-3 py-1 text-xs font-medium text-slate-200">{t('plans.badge')}</span>
          <h1 className="mt-4 text-4xl font-extrabold tracking-tight sm:text-5xl">
            {t('plans.title1')} <span className="text-brand-gradient-light">{t('plans.title2')}</span>
          </h1>
          ...
          {plans.map((plan, i) => {
            ...
            return (
              <div key={plan.id} className={`relative flex flex-col rounded-3xl bg-white p-7 shadow-card ${popular ? 'border-2 border-brand-500 shadow-glow' : 'border border-slate-200/80'}`}>
                {popular && <span className="bg-brand-gradient absolute -top-3 left-1/2 -translate-x-1/2 rounded-full px-3 py-1 text-xs font-bold text-white">{t('plans.mostPopular')}</span>}
                ...
                <div className="mt-5 text-4xl font-extrabold text-navy-900">
                  ...
                  {plan.priceCents > 0 && <span className="text-sm font-medium text-slate-500"> {plan.interval === 'YEAR' ? t('common.perYear') : t('common.perMonth')}</span>}
                </div>
                <ul className="mt-6 space-y-3 text-sm text-slate-700">
                  <Feature>{t('plans.commission', { pct: formatBps(plan.commissionRateBps) })}</Feature>
                  <Feature>{plan.maxProducts === null ? t('plans.unlimitedProducts') : t('plans.listedProducts', { count: plan.maxProducts })}</Feature>
                  <Feature>{t(plan.withdrawalsPerWeek > 1 ? 'plans.withdrawalsPlural' : 'plans.withdrawals', { count: plan.withdrawalsPerWeek })}</Feature>
                ...
                >
                  {t('plans.choose', { name: plan.name })} <ArrowRightIcon size={16} className="arrow-nudge" />
                </Link>
              </div>
            );
          })}
        </div>
        {plans.length === 0 && <p className="mt-10 text-center text-sm text-slate-500">{t('plans.noPlans')}</p>}
      </PageContainer>
    </>
  );
}
