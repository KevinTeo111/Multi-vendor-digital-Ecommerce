...
export default async function ProductPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const previews = [product.thumbnailUrl, ...product.previewImageUrls].filter((u): u is string =>
    Boolean(u),
  );
        ...
        <div>
          <div className="overflow-hidden rounded-3xl border border-slate-200/80 bg-navy-900 shadow-card">
            {previews[0] ? (
              ...
              <div className="bg-hero flex aspect-[4/3] items-center justify-center text-slate-400">
                {t('product.noPreview')}
              </div>
            )}
          </div>
          ...
          <div className="mt-8 rounded-2xl border border-slate-200/80 bg-white p-6 shadow-card">
            ...
            <article className="whitespace-pre-wrap text-sm leading-6 text-slate-700">
              {product.description}
            </article>
              ...
              <div className="mt-5 flex flex-wrap gap-2">
                {product.tags.map((tag) => (
                  <Link
                    ...
                    className="rounded-full bg-slate-100 px-3 py-1 text-xs text-slate-600 hover:bg-brand-50 hover:text-brand-700"
                  >
                    #{tag}
                  </Link>
                ))}
        ...
        <div className="space-y-5">
          <div className="rounded-3xl border border-slate-200/80 bg-white p-6 shadow-card">
            ...
            <ul className="mt-5 space-y-2 text-sm text-slate-700">
              ...
              {product.files.length > 0 && (
                <Feature>
                  {t(product.files.length === 1 ? 'product.files' : 'product.filesPlural', {
                    count: product.files.length,
                    size: formatBytes(totalBytes),
                  })}
                </Feature>
              )}
              {product.version && (
                <Feature>{t('product.version', { version: product.version })}</Feature>
              )}
            </ul>
            ...
            <div className="mt-6 space-y-2">
              ...
              {product.demoUrl && (
                <a
                  ...
                  target="_blank"
                  rel="noreferrer"
                  className="flex h-11 w-full items-center justify-center rounded-full border border-slate-200 text-sm font-semibold text-slate-700 hover:border-brand-500 hover:text-brand-600"
                >
            ...
            <div className="mt-5 flex items-center gap-4 border-t border-slate-100 pt-4 text-xs text-slate-500">
              <span className="flex items-center gap-1">
                <ShieldIcon size={14} className="text-brand-500" /> {t('product.securePayment')}
              </span>
              <span className="flex items-center gap-1">
                <DownloadIcon size={14} className="text-brand-500" />{' '}
                {t('product.protectedDownload')}
              </span>
            </div>
          </div>
          ...
          <div className="rounded-2xl border border-slate-200/80 bg-white p-5 shadow-card">
            ...
            <div className="flex items-center gap-3">
              {product.vendor.logoUrl ? (
                ...
                <span className="bg-brand-gradient flex h-12 w-12 items-center justify-center rounded-xl text-lg font-bold text-white">
              ...
              <Link
                href={`/store/${product.vendor.slug}`}
                className="flex items-center gap-1 rounded-full border border-slate-200 px-3 py-1.5 text-xs font-semibold text-slate-700 hover:border-brand-500 hover:text-brand-600"
              >
          ...
          <div className="rounded-2xl border border-slate-200/80 bg-white p-5 shadow-card text-sm">
            ...
            <dl className="space-y-2">
              ...
              <Row label={t('product.published')}>
                {formatDate(product.publishedAt, false, locale)}
              </Row>
              {product.files.map((f) => (
