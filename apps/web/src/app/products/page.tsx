...
export default async function ProductsPage({ searchParams }: { searchParams: Promise<Search> }) {
  ...
  const PRICE_RANGES: Array<{ label: string; min?: number; max?: number }> = [
    ...
    {
      label: t('browse.between', { min: formatMoney(2500), max: formatMoney(10000) }),
      min: 2500,
      max: 10000,
    },
  ...
  const title = params.search
    ? t('browse.resultsFor', { query: params.search })
    : (activeCategory?.name ?? t('browse.allProducts'));
  ...
  return (
    <PageContainer>
      <nav className="mb-4 text-xs text-slate-500">
        ...
        <span className="mx-1">/</span> <span className="text-slate-700">{t('browse.title')}</span>
        {activeCategory && (
          <>
        ...
        <aside className="space-y-6">
          ...
          <div className="rounded-2xl border border-slate-200/80 bg-white p-4 shadow-card">
            ...
            <ul className="space-y-1 text-sm">
              {PRICE_RANGES.map((r) => {
                const active =
                  String(params.minPriceCents ?? '') === String(r.min ?? '') &&
                  String(params.maxPriceCents ?? '') === String(r.max ?? '');
                return (
                  <li key={r.label}>
                    <FilterLink
                      href={link({
                        minPriceCents: r.min?.toString(),
                        maxPriceCents: r.max?.toString(),
                        page: undefined,
                      })}
                      active={active}
                    >
          ...
          <div className="mb-5 flex flex-wrap items-center justify-between gap-3">
            <div>
              ...
              <p className="text-sm text-slate-500">
                {t('browse.productsCount', { count: result?.total ?? 0 })}
              </p>
            </div>
            <Suspense>
            ...
            <>
              ...
              {result.totalPages > 1 && (
                <nav className="mt-8 flex items-center justify-between text-sm">
                  {page > 1 ? (
                    <PageLink href={link({ page: String(page - 1) })}>
                      ← {t('common.previous')}
                    </PageLink>
                  ) : (
                    <span />
                  ...
                  <span className="text-slate-500">
                    {t('common.pageOf', { page, total: result.totalPages })}
                  </span>
...
function FilterLink({
  ...
  return (
    <Link
      href={href}
      className={`flex items-center gap-2 rounded-full px-3 py-2 transition ${active ? 'bg-brand-500 font-semibold text-white' : 'text-slate-700 hover:bg-slate-50'}`}
    >
...
function PageLink({ href, children }: { href: string; children: React.ReactNode }) {
  return (
    <Link
      href={href}
      className="rounded-full border border-slate-200 bg-white px-4 py-2 font-medium text-slate-700 shadow-sm hover:border-brand-500 hover:text-brand-600"
    >
      {children}
    </Link>
  );
